...
class TextSumFromFullArticle(spark:SparkSession) {
  ...
  var keywordsArticleDF:DataFrame = _

  def getData(df:DataFrame): Unit ={

    keywordsArticleDF = df.select("_id","keywords_extracted.result","text")

    keywordsArticleDF = applyTextSum()
  ...
  def applyTextSum():DataFrame = {

    val _kwa = keywordsArticleDF

    val kwaTriple = _kwa.map(x => (x.getString(0),x.getAs[List[String]](2),x.getString(1)))

    kwaTriple.map{ case(id,keys,article) => textSum(id,keys,article)}.toDF("_id","textsum")
